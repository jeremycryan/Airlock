...
class OxygenCells(object):
    ...
    def __init__(self, oxygen_profile, screen):
        ...
        surf_width = self.start_count * CARDSIZE[0] + \
            (self.start_count - 1) * OXYGEN_SPACING
        surf_height = CARDSIZE[1]
        self.oxygen_surface = pygame.Surface((surf_width, surf_height))

        #   Create card object for each oxygen cell
        for idx, item in enumerate(self.oxygen_profile):
            xpos = idx * CARDSIZE[0] + idx * OXYGEN_SPACING
            ypos = 0
    ...
    def get_oxygen_surface(self):
        ...
        self.oxygen_surface.fill((0, 0, 0))
